...
const matrix={
  ...
  start : function(){
    ...
    this.key=[false, false, false, false]; this.time=[-1, -1, -1, -1]; this.pause=false; this.savetime; this.stop=0;
    this.interval=setInterval(reset,20);
  }
}
function item(x, y, w, h, color){
  this.x=x; this.y=y; this.w=w; this.h=h; this.color=color; this.turn=0;
  ...
  this.falls=function(){
    if(++this.turn>60) this.turn=1;
    matrix.context.save();
    matrix.context.translate(this.x+this.w/2, this.y+this.h/2);
    matrix.context.rotate(this.turn*Math.PI/30);
    matrix.context.beginPath();
    matrix.context.fillStyle=this.color;
    matrix.context.fillRect(-this.w/2, -this.h/2, this.w, this.h);
    matrix.context.restore();
  }
}
function rand(v){
  let piece=Math.floor((canvaswidth-debriswidth))/15;
  if(v==1) return Math.floor(Math.random()*piece)+3*piece;
  if(v==2) return Math.floor(Math.random()*piece)+11*piece;
  if(v==3) return Math.floor(Math.random()*piece);
  if(v==4) return Math.floor(Math.random()*piece)+14*piece;
  if(v==5) return Math.floor(Math.random()*piece)+5*piece;
  if(v==6) return Math.floor(Math.random()*piece)+9*piece;
  if(v==7) return Math.floor(Math.random()*piece)+2*piece;
  if(v==8) return Math.floor(Math.random()*piece)+12*piece;
  if(v==9) return Math.floor(Math.random()*piece)+6*piece;
  if(v==10) return Math.floor(Math.random()*piece)+8*piece;
  if(v==11) return Math.floor(Math.random()*piece)+piece;
  if(v==12) return Math.floor(Math.random()*piece)+13*piece;
  if(v==13) return Math.floor(Math.random()*piece)+4*piece;
  if(v==14) return Math.floor(Math.random()*piece)+10*piece;
  if(v==15) return Math.floor(Math.random()*piece)+7*piece;
}
function countdown(){
...
function reset(){
  ...
  for(i=0; i<debris.length; i++){
   ...
   if(++matrix.count%30==0){
     if(++matrix.eleven>15) matrix.eleven=1;
...
function stop(){
  ...
  matrix.context.beginPath();
  matrix.context.fillStyle='rgba(100,100,100,0.5)'
  matrix.context.fillRect(0, 0, matrix.canvas.width, matrix.canvas.height);
  matrix.context.beginPath();
  matrix.context.fillStyle='#FFCC33'
  matrix.context.fillRect(matrix.canvas.width/2-100, matrix.canvas.height/2-50, 200, 100);
  matrix.context.beginPath();
